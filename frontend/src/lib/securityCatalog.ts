...
export type LegacySecurityRule = {
  key: string;
  name: string;
  description: string;
...
export type LegacySecurityResolvedMap = Record<string, { minLevel: number; enforced: boolean }>;

export const PERMISSION_OPTIONS: PermissionOption[] = [
  { key: "all", label: "Full Access", hint: "Grants all protected access." },
  { key: "orders", label: "Orders & Kitchen", hint: "Order entry, recall, and kitchen routes." },
  { key: "menu", label: "Menu Management", hint: "Menu, modifiers, taxes, and discounts." },
  { key: "tables", label: "Tables & Floor", hint: "Table setup, floor plan, hostess assignments." },
  { key: "cash", label: "Cash Operations", hint: "Cash drawer, house accounts, payouts." },
  { key: "reports", label: "Reports & Owner", hint: "Reporting dashboards and owner views." },
  { key: "inventory", label: "Inventory", hint: "Inventory, vendors, purchase orders." },
  { key: "users", label: "Users & Roles", hint: "User accounts and role maintenance." },
  { key: "settings", label: "System Settings", hint: "Global settings, stations, maintenance." },
  { key: "timeclock", label: "Time Clock", hint: "Time clock and shift access." }
];

export const LEGACY_SECURITY_RULES: LegacySecurityRule[] = [
  {
...
export function normalizePermissionMap(value: unknown): Record<string, boolean> {
...
export function normalizeLegacySecurityOverrides(value: unknown): LegacySecurityRuleOverrideMap {
  ...
  for (const [key, raw] of Object.entries(value)) {
    ...
    const rule: LegacySecurityRuleOverride = {};
    if (typeof raw.minLevel === "number" && Number.isInteger(raw.minLevel) && raw.minLevel >= 1 && raw.minLevel <= 5) {
      rule.minLevel = raw.minLevel;
    }
    if (typeof raw.enforced === "boolean") {
      rule.enforced = raw.enforced;
    }
    if (typeof rule.minLevel === "undefined" && typeof rule.enforced === "undefined") continue;
    next[key] = rule;
  }
  return next;
}

export function resolveLegacyRuleConfig(rule: LegacySecurityRule, override?: LegacySecurityRuleOverride) {
  return {
    minLevel: override?.minLevel ?? rule.defaultMinLevel,
    enforced: override?.enforced ?? rule.defaultEnforced
  };
}

export function buildResolvedLegacyConfig(overridesValue: unknown): LegacySecurityResolvedMap {
  const overrides = normalizeLegacySecurityOverrides(overridesValue);
  const resolved: LegacySecurityResolvedMap = {};
  for (const rule of LEGACY_SECURITY_RULES) {
    resolved[rule.key] = resolveLegacyRuleConfig(rule, overrides[rule.key]);
  }
  return resolved;
}

export function buildLegacyOverridesFromResolved(resolved: LegacySecurityResolvedMap): LegacySecurityRuleOverrideMap {
  const overrides: LegacySecurityRuleOverrideMap = {};
  for (const rule of LEGACY_SECURITY_RULES) {
    const current = resolved[rule.key];
    if (!current) continue;
    const next: LegacySecurityRuleOverride = {};
    if (current.minLevel !== rule.defaultMinLevel) {
      next.minLevel = current.minLevel;
    }
    if (current.enforced !== rule.defaultEnforced) {
      next.enforced = current.enforced;
    }
    if (typeof next.minLevel !== "undefined" || typeof next.enforced !== "undefined") {
      overrides[rule.key] = next;
    }
  }
  return overrides;
}
...
export function evaluateLegacySecurity(
  resolvedConfigValue: unknown,
  securityLevelValue: unknown
): Record<string, { minLevel: number; enforced: boolean; allowed: boolean }> {
  const resolvedConfig = buildResolvedLegacyConfig(resolvedConfigValue);
  ...
  for (const rule of LEGACY_SECURITY_RULES) {
    ...
    evaluated[rule.key] = { ...current, allowed };
  }
